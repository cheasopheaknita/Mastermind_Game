...
"""
    Evaluate the correctness of a guess against the secret code.

    Parameters:
    - secret_code (list): The secret code generated for the game.
    - guess (list): The user's guess for the secret code.

    Returns:
    - Tuple: A tuple containing two values:
        - correct_color_and_position (int): Number of correct colors in correct positions.
        - correct_color_only (int): Number of correct colors in incorrect positions.
    """
...
def evaluate_guess(secret_code, guess):
    ...
    # by subtracting correct_color_and_position from the total correct color count
...
def reset_game():
    # Function to reset the game, generating a new secret code and resetting attempts
...
reset_game()  # Initial game setup
 ...
 # Route to render the initial game page
@app.route('/')
def index():
    return render_template('mastermind_game.html', secret_code=secret_code)

# Route to handle the submission of user guesses
